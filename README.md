...
- **서드파티 아이콘**: 일부 범용 아이콘(깃발·비행기·해/달·자물쇠 등)은 [Lucide](https://lucide.dev)(ISC/MIT)의
  ...
  라이선스 전문은 [`public/vendor/lucide/LICENSE`](public/vendor/lucide/LICENSE)에 있다.
...
| **게임 보드** | 11×11 그리드 외곽 40칸의 정사각 보드. 칸마다 이름 · 가격/통행료 · 소유자 색 띠+무늬 · **건물 배지 3칸(별/빌/호, 지은 것만 채움) · 금색 ★ 관광명소 리본**. 말은 칸 위 **오버레이 층**에 크게 그려지고(현재 차례는 통통 튐, 내 말은 "나" 배지) 주사위 이동은 **칸을 하나씩 밟아 간다**. 칸을 누르면 상세 시트(소유자 · 건물별 지음/안 지음 · 현재 통행료 · 인수 가격) |
| **범례 · 모바일 상황판** | 보드 아래 범례(건물 배지·플레이어 색/모양). 폰에서는 보드 아래 고정 상황판에 차례 · 📍 현재 칸 · 주사위 · "내 위치" · "확대"(보드 170%) |
| **중앙 코어** | 라운드 · 잭팟 · 현재 차례와 페이즈 안내 · **📍 현재 위치** · 주사위(눈 + 숫자 배지, 더블 표시) · 주 행동 버튼. 넓은 화면에서는 보드 안쪽 아래쪽에(위 띠에는 엠블럼이 보인다), 세로 화면에서는 보드 아래에 놓인다 |
...
처음부터 호텔까지 지을 수는 없다. **지금 몇 바퀴째인가**가 지을 수 있는 건물을 정한다.

| 바퀴 | 지을 수 있는 건물 | 비고 |
|---|---|---|
| 1바퀴 | 별장 | |
| 2바퀴 | 별장 · 빌딩 | |
| 3바퀴 이상 | 별장 · 빌딩 · 호텔 | 3종을 갖춘 **다음** 기회에 관광명소 |

- 바퀴는 출발 칸을 **앞으로 지나거나 도착해 월급을 받을 때마다** 1씩 오른다(월급이 대출 상환에 압류돼도 오른다). 뒤로 밀려 출발 칸에 도착하거나 조난 섬으로 이송되면 오르지 않는다.
- 종류 사이의 순서는 없다(3바퀴면 별장 없이 호텔만 지어도 되고, 세 종류를 한 번에 지어도 된다).
- 아직 못 짓는 건물은 건설 창에 🔒 잠긴 행으로 보이고 "3바퀴부터 지을 수 있습니다"로 안내한다. 그 바퀴에 지을 것이 하나도 없으면 건설 창이 아예 열리지 않는다.
- 인수한 도시의 건물은 인수자의 바퀴와 무관하게 그대로 남는다(3종이 완성된 도시를 인수하면 곧바로 관광명소를 지을 수 있다).
...
- 테마: 다크/라이트 모드. 홈 화면과 게임 상단바의 작은 아이콘 버튼으로 자동 → 라이트 → 다크 순서로 순환(자동은 기기 설정을 따름). 선택은 이 기기에 저장되고, 브라우저가 강제로 다크를 입히는 경우(삼성 인터넷 등)에도 이중 반전 없이 고른 테마 그대로 보인다. 보드 칸·카지노 화면·행운 티켓 뒷면은 실제 게임판처럼 테마와 무관하게 항상 같은 톤이다.

## 투자 모드 — 증권거래소 (선택)

호스트가 **대기실에서** 켜는 선택 기능이다. 기본값은 꺼짐이며, 켜지 않으면 게임은 예전과 완전히 똑같다.

### 켜는 방법

대기실 호스트 도구에서 **투자 모드**를 `증권거래소`로 바꾸고 게임을 시작한다. 시장 패널은 플레이어 목록 위(넓은 화면은 옆 칸, 폰은 코어 바로 아래)에 나타난다.
(API로는 `POST /api/rooms/:code/host-actions`에 `{ "type": "SET_OPTIONS", "roundLimit": 30,
"finance": { "investmentMode": "STOCKS" } }`.)

- **대기실에서만** 바꿀 수 있다. 시작 시점의 옵션이 판 내내 고정된다.
- 이미 저장된 방(업데이트 전에 시작한 판)은 **꺼진 상태로 그대로 이어진다**.

### 규칙 요약

- **거래 시간**: 자기 차례가 시작되면 주사위를 굴리기 **전에** 거래 창구가 한 번 열린다.
  조난 중에도 열리고(허탕 치는 턴을 만들지 않는다), 더블로 얻은 추가 턴에는 열리지 않는다.
  현금·보유·예금이 모두 0이면 열리지 않는다.
- **주문 건수·금액 제한 없음.** 원하는 만큼 사고판 뒤 "거래 마치고 주사위 굴리기"를 누른다(종목당 500주 보유 상한만 있다).
  주문 1건은 1~200주, 한 종목은 최대 500주까지 가질 수 있다(금액 상한 없음).
- **수수료는 사고팔 때 각각** 명목금액의 1%(최소 1,000원)다. 그래서 같은 창구 안에서
  사고 되팔면 **언제나 손해**다 — 단타로 돈을 벌 수 없게 되어 있다.
- **시세는 주문에 반응하지 않는다.** 경기 국면(호황·과열·침체·회복)과 그 라운드의 경제 뉴스,
  그리고 서버 난수만이 가격을 움직인다. 누가 많이 산다고 값이 오르지 않으므로 시세 조작이 불가능하다.
- **뉴스는 국면에서 나온다.** 국면마다 6장씩 24장이 있고, 침체에 호황 뉴스가 나오는 일은 없다.
  카드마다 "왜 그런지" 한 줄 설명이 붙는다.
- **보드에서 벌어진 일이 다음 라운드 시세에 반영된다.** 랜드마크를 지으면 건설주, 잭팟이 터지면
  카지노주가 오른다. 단 **다음 라운드에 반영되고 전원에게 미리 공개**되므로, 내 턴에 미리 사두는
  식의 내부정보 거래가 성립하지 않는다.
- **배당**은 출발 칸을 지날 때 월급과 함께 받는다(상장 종목만). 종목 카드와 주문 미리보기에 "출발 통과 시 +N원(1주 M원 × 보유 주수)"이 미리 표시되고, 받을 때 월급처럼 안내 카드가 뜬다.
- **예금**은 10,000원 단위로 1,000만원까지. 이자는 라운드마다 기준금리만큼 붙지만,
  **대출이 있으면 이자가 0**이다(빌린 돈을 예금에 넣어 이자만 챙기는 방법은 막혀 있다).
- **상장폐지**: 기준가의 20%까지 떨어진 종목은 상장폐지되고 **보유 수량은 휴지조각이 된다**.
  다음 라운드에 새 회사가 상장되어 목록은 항상 5종목이다. 한 종목에 몰아넣지 않는 것이 안전하다.
- **예약 주문**: 남의 차례에도 자기 주문을 미리 걸어 둘 수 있다(좌석당 3건). 내 차례가 시작되면
  자동으로 체결되며, 그 순간의 현금·한도·상장 여부를 다시 검사해 안 되면 그 주문만 취소된다.
- **현금이 부족해 정리해야 할 때**는 주식 → 예금 → 부동산 순서로 팔린다. 이때는 **수수료가 면제**된다.
- **총자산**에 주식 평가액과 예금이 포함된다. 플레이어 패널에서 `현금 / 부동산 / 주식 / 예금`으로
  나눠 볼 수 있다.

### 밸런스 (300시드 × 25라운드 시뮬레이션)

5종목을 고르게 사서 들고만 있으면 **한 바퀴에 평균 +6.2%**, 25라운드에 평균 +31%(배당까지 더하면
+43%)다. 그렇다고 확실한 돈은 아니다 — 25라운드 뒤에 손실인 경우가 **12%**, 한 바퀴 구간이
하락인 경우가 **35%**다. 예금만 하면 25라운드에 +15% 정도이므로, 위험을 지는 쪽이 평균적으로 더
번다(대신 흔들린다). 한 종목만 들면 손실 확률이 16~34%로 뛴다 — **분산이 안전하다.**

숫자를 직접 확인하려면:

```bash
node scripts/dumpMarket.js --seed 42 --rounds 25   # 라운드별 시세·국면·뉴스 표
node scripts/dumpMarket.js --summary --seeds 300   # 밸런스 통계
```

자세한 규칙과 상수는 `docs/SPEC.md` 12장, 서버 계약은 `docs/API.md` 9장에 있다.
...
src/
  ...
  domain/                    순수 비즈니스 로직 — node:*/브라우저 API 의존 금지
    ...
    market/                  증권거래소(투자 모드 STOCKS) — 모든 메서드가 {intents, events}만 돌려준다
      Market.js              시장 루트: 라운드 틱 순서·거래 창구·예약 주문·배당·정리 위임
      Instrument.js          상품 엔티티(가격/이력/상장폐지/배당) — 자기 가격 규칙을 안다
      PriceProcess.js        틱 규칙(순수 함수, 상품 종류 무관): drift+news+nudge+shock
      PriceSeries.js         가격 이력 VO(최근 30개, 스파크라인 입력)
      BusinessCycle.js       호황→과열→침체→회복 상태기계
      BaseRate.js            기준금리 VO(25~400bp, 뉴스만이 움직인다)
      NewsDeck.js            국면별 독립 덱 비복원 추출
      Holdings.js            보유 포지션(이동평균 평단, 종목별 500주 상한)
      DepositAccount.js      예금(이자는 예금 − 총부채에만 붙는다)
      TradingDesk.js         ★ 주문 검증·체결·수수료 — 거래의 유일한 진입점
      TradeBudget.js         창구 예산 VO(현재 상한 없음 — D46)
      OrderQueue.js          남의 턴 예약 주문(체결 시점 전면 재검증)
      MarketAssets.js        AssetProvider 구현(주식 30 · 예금 50)
      BoardNudges.js         보드 사건 → 섹터 압력 표(다음 틱 반영)
      MarketSimulation.js    밸런스 시뮬레이터(실제 roundTick을 돌린다)
      data/                  창작 종목 5종 + 예비 3종 · 경제 뉴스 24장 · 국면 파라미터
    report/
      MatchRecorder.js       ★ Phase 1부터 수집: 라운드 자산 스냅샷 + 하이라이트 + 사유별 손익
      Board.js  City.js      40칸 보드, 도시/휴양지(건물·통행료·투자액·인수·매각·바퀴별 선택지)
      buildings.js           건물 3종 + 관광명소, 바퀴별 해금 규칙(BuildingUnlocks)
...
public/                      클라이언트 (빌드 없음, 의존성 0)
  ...
  js/
    ...
    views/                   home · lobby · board · center · players · log · game · casino · toast
                             dice(공용 주사위) · statusStrip(모바일 상황판) · legend(범례)
...
- **풍부한 도메인 모델**: 규칙 판단은 엔티티/애그리거트와 도메인 서비스 안에 있다(`City.tollFor/buildOffer/liquidationValue`, `BuildingUnlocks`가 바퀴별 해금, `CityTrade`가 매입·건설·인수, `TicketEffects`가 티켓 효과, `Liquidator`가 매각 순서, `Game`이 페이즈·턴 소유권 강제, `Room`이 호스트 권한·좌석 한도 강제). 애플리케이션 서비스는 조합만 한다.
...
- **규칙은 도메인에만**: 바퀴별 건설 제한 같은 규칙은 `Player.lap` · `City.buildableTypes({lap})` · `BuildingUnlocks`에만 있고, 서비스와 클라이언트는 서버가 준 선택지(`options` / `lockedOptions`)를 그리기만 한다. 잠긴 건물을 담은 커맨드는 서버가 규격 에러로 거부한다.
